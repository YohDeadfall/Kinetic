...
internal abstract class KineticPropertyDescriptor : PropertyDescriptor
{
    ...
    {
        static KineticPropertyDescriptor<T> Factory<T>(PropertyDescriptor property) =>
        ...
        return
            ...
            genericType.GetGenericArguments() is [var actualType]
            ? Reflection
...
internal sealed class KineticPropertyDescriptor<T> : KineticPropertyDescriptor
{
    ...
    internal KineticPropertyDescriptor(PropertyDescriptor parent) :
        base(parent)
    {
        _accessor = ComponentType.GetProperty(Name)?.GetMethod is { } getter
            ? IsReadOnly
